...
# ── 11. Module AWS WAF (Sécurité de l'ALB) ───────────────────────────────────
module "waf" {
  source      = "./modules/waf"
  name_prefix = local.name_prefix
  alb_arn     = module.alb.alb_arn # L'ARN de l'ALB exposé par votre module 'alb'
}

# ── 12. Module Amazon CloudFront (CDN & HTTPS automatique) ───────────────────
module "cloudfront" {
  source       = "./modules/cloudfront"
  name_prefix  = local.name_prefix
  alb_dns_name = module.alb.alb_dns_name # Le DNS public de votre ALB
}